...
def train(**kwargs):
    ...
    model=getattr(models,opt.model)()       # model为网络结构，注意等式右边的括号，如果没有括号，则调用网络需要使用model()
    # 如果有预训练好的网络，则采用之
    ...
    # 步骤2：加载数据
    # 加载训练集数据
    train_data=DogCat(opt.train_data_root,train=True)       # 使用自定义数据集类DogCat加载数据，使用train_data.__getitem__(index)获取对应索引的data和label
    train_dataloader=DataLoader(train_data,opt.batch_size,shuffle=True,num_workers=opt.num_workers)     # 使用PyTorch自带的utils.data中的DataLoader加载数据
    # 加载验证集数据
    val_data=DogCat(opt.train_data_root,train=False)
    # DataLoader是一个可迭代对象，可将dataset返回的每条数据拼接成一个batch，并提供多线程加速优化，当所有数据遍历完一次，对DataLoader完成一次迭代
    val_dataloader=DataLoader(val_data,opt.batch_size,shuffle=False,num_workers=opt.num_workers)        # DataLoader提供对数据分批，打乱，使用多少子进程加载数据
    ...
    confusion_matrix=meter.ConfusionMeter(2)    # 多类之间的混淆矩阵，初始化时，指定类别数为2,参数normalized指定是否归一化名，归一化后输出百分比，否则输出数值
    previous_loss=1e100
